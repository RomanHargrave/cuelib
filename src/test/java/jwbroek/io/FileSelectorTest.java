...
public class FileSelectorTest {
    ...
    public void setUp() throws SecurityException, IOException {
        ...
        for (int fileIndex = firstFile; fileIndex < fileBound; fileIndex++) {
            TemporaryFileCreator.createNamedTemporaryFileOrDirectory(FileSelectorTest.rootFilePrefix + fileIndex, "", this.testRoot, false, true);
        }
    ...
    public static Pattern getParentDirNameAsFileNamePattern() {
        ...
        final StringBuilder patternHelper = new StringBuilder(".*").append(patternDirSeparator).append("([^").append(patternDirSeparator).append("]*)").append(patternDirSeparator).append("\\1[^").append(patternDirSeparator).append("]*").append(patternDirSeparator).append("?$");
    ...
    public void testUnionFileFilterFromArray() {
        ...
        final FileFilter unionFileFilter = FileSelector.getUnionFileFilter(FileSelector.getFileNamePatternFilter("b1"), FileSelector.getFileNamePatternFilter("b3"));
        final Set<File> prediction = new HashSet<File>();
    ...
    public void testSelectFilesWithDepthZeroNoBaseFile() {
        ...
        testFilesAgainstPrediction(matchedFiles, prediction, "FileSelector.selectFiles(File,Pattern,boolean,boolean); depth zero, no consideration of base file");
    }
    ...
    public void testSelectFilesWithDepthOneAndNameStartingWithRootName() {
        ...
        prediction.add(new File(this.testRoot, this.testRoot.getName() + "file"));
        prediction.add(new File(this.testRoot, this.testRoot.getName()));
        testFilesAgainstPrediction(matchedFiles, prediction, "FileSelector.selectFiles(File,Pattern,boolean,boolean);" + " all files at depth one of which the name starts with the name of the root");
    }
    ...
    public void testSelectFilesWithNameStartingWithParentName() {
        ...
        matchedFiles = FileSelector.selectFiles(this.testRoot, FileSelectorTest.getParentDirNameAsFileNamePattern(), Integer.MAX_VALUE, false, true);
        prediction = new HashSet<File>();
        ...
        prediction.add(new File(this.testRoot, this.testRoot.getName()));
        prediction.add(new File(this.testRoot, this.testRoot.getName() + "file"));
        testFilesAgainstPrediction(matchedFiles, prediction, "FileSelector.selectFiles(File,Pattern,boolean,boolean); " + "all files of which the name starts with the name of the root");
    }
    ...
    public void testSelectFilesFromFileFilter() {
        ...
        testFilesAgainstPrediction(matchedFiles, prediction, "FileSelector.selectFiles(File, FileFilter, List, long, boolean, boolean); " + "all files, but not directories");
    }
    ...
    private void testFilesAgainstPrediction(final List<File> filesFound, final Set<File> predictedResult, final String methodDescription) {
        ...
        // highly undesirable.
        Assert.assertEquals("List of files found by '" + methodDescription + "' contains duplicates.", filesFound.size(), matchedFiles.size());

        // All matched files must be in the prediction.
        for (File acceptedFile : filesFound) {
            Assert.assertTrue(methodDescription + " accepted the unpredicted file: '" + acceptedFile.toString() + "'", unmatchedFiles.contains(acceptedFile));
