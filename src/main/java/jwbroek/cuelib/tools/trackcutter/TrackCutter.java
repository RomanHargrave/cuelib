...
public class TrackCutter {
    ...
    public void cutTracksInCueSheet(final File cueFile) throws IOException {
        ...
        if (getConfiguration().getParentDirectory() == null) {
            ...
            TrackCutter.logger.debug("Have set base directory to directory of File  '{}'.", cueFile.toString());
        }

        try {
            TrackCutter.logger.trace("Parsing cue sheet.");
            cueSheet = CueParser.parse(cueFile);
        } catch (IOException e) {
            TrackCutter.logger.error("Was unable to parse the cue sheet in file '{}'.", cueFile.toString(), e);
    ...
    public void cutTracksInCueSheet(final InputStream inputStream) throws IOException {
        ...
        try {
            ...
            TrackCutter.logger.error("Was unable to parse the cue sheet from InputStream.", e);
            IOException resultException = new IOException("Problem parsing cue file.");
            resultException.initCause(e);
            throw resultException;
        }
    ...
    private void cutTracksInFileData(final FileData fileData) throws IOException, UnsupportedAudioFileException {
        ...
        try {
            ...
            TrackCutter.logger.trace("Determining complete path to audio file.");
            ...
            TrackCutter.logger.trace("Opening audio stream.");
            ...
            if (audioInputStream != null) {
                ...
                TrackCutter.logger.trace("Closing audio stream.");
    ...
    private List<TrackCutterProcessingAction> getProcessActionList(final FileData fileData) {
        TrackCutter.logger.debug("Determining processing actions for file: '{}'.", fileData.getFile());
     ...
     *                       will be added.
     */
    private void addProcessActions(final TrackData trackData, final Position nextPosition, final List<TrackCutterProcessingAction> processActions) {
        TrackCutter.logger.debug("Adding processing action for track #{}.", trackData.getNumber());
    ...
    private long performProcessAction(final TrackCutterProcessingAction processAction, final AudioInputStream audioInputStream, final long currentAudioFramePos) throws IOException {
        TrackCutter.logger.debug("Determining audio substream for processing action for {} of track #{}.", (processAction.getIsPregap() ? "pregap" : "contents"), processAction.getTrackData().getNumber());
    ...
    private void performProcessAction(final TrackCutterProcessingAction processAction, final AudioInputStream audioInputStream) throws IOException {
        TrackCutter.logger.info("Performing processing action for {} of track #{}.", (processAction.getIsPregap() ? "pregap" : "contents"), processAction.getTrackData().getNumber());
        ...
        if (!getConfiguration().getRedirectToPostprocessing()) {
            ...
            TrackCutter.logger.debug("Creating directory for target files.");
        ...
        if (configuration.getDoPostProcessing() && configuration.getRedirectToPostprocessing()) {
            ...
            try {
                TrackCutter.logger.debug("Writing audio to postprocessor.");
                ...
                if (audioOutputStream != null) {
                    ...
                    TrackCutter.logger.debug("Closing audio stream.");
                    audioOutputStream.close();
                }
            }
        } else {
            TrackCutter.logger.debug("Writing audio to file.");
            ...
            if (configuration.getDoPostProcessing()) {
                TrackCutter.logger.debug("Performing postprocessing.");
    ...
    private Process createPostProcessingProcess(final TrackCutterProcessingAction processAction) throws IOException {
        TrackCutter.logger.debug("Creating post-processing process for command: {}", processAction.getPostProcessCommand());
